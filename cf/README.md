...
从阶段 2 起，翻译站不再只靠"共享口令"，而是**要求登录**：

| | 说明 |
|---|---|
| 登录 | 与账户站（account.ourmetaverse.cn）**共用同一份会话**：Cookie 作用域是 `.ourmetaverse.cn`，两边绑同一个 D1。在账户站登录一次，翻译站直接认 |
| 额度 | 每人每月默认 200 页（管理员可在账户站后台调整）。每翻完一个文件，按**实际页数**扣额度并写一条用量流水 |
| 额度用完 | 上传直接被拒（HTTP 402），提示"下个月自动恢复或找管理员加额度" |
| 任务归属 | 只能看自己的任务与历史；读别人的任务返回 403（管理员例外） |
| 管理口令 | 原来的共享口令**保留**，但降级为"管理员/自动化通道"（线上自检用它，不占任何人额度） |

实现要点：`cf/shared/session.js` 是两个 Worker 共用的会话/额度模块（安全逻辑只有一份）；
翻译站的 `wrangler.toml` 里绑了与账户站**同一个 D1**（只读会话、写用量）。

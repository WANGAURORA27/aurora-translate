...
| 角色 | 默认额度 | 怎么来的 |
|---|---|---|
| `admin` | **200 页/月** | 第一个注册的账号自动是管理员 |
| `vip` | **50 页/月** | 注册时填了有效**邀请码**，自动成为 VIP |
| `user` | **30 页/月** | 普通注册（不填邀请码） |

* 管理员在后台点「改」只换角色时，会**自动套用该角色的默认额度**；显式填了数字就以填的为准（`0` = 不限）。
* **邀请码**：管理员后台「邀请码」卡片 → 生成（可指定一次生成几个 / 每个可用几次 / 有效天数），码形如 `AURORA-934J-EKK8`（去掉了容易看错的 I O 0 1）。朋友注册时填上即自动 VIP。
* 想让**注册必须凭邀请码**（把邀请码当口令用）：给账户 Worker 设一个变量 `REQUIRE_INVITE=1` 即可，此时不填码无法注册。
* 邀请码是一次性消费的：用完（`used_count` 达到 `max_uses`）或过期后自动失效，接口会明确告知原因。
